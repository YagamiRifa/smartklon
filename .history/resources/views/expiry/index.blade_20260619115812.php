...
async function openExpiryDetailModal(itemId) {
    ...
    try {
        ...
        // 1. Ambil nama barang untuk dikirim ke form edit
        ...
        d.batches.forEach(b => {
            ...
            html += `
                <tr style="border-bottom: 1px solid var(--grey-100);">
                    ...
                    <td style="padding:10px 12px; text-align:center;">
                        <button class="btn-row-icon btn-row-icon--edit" style="width:24px; height:24px; font-size:11px;"
                                onclick="closeExpiryDetailModal(); openEditModal('${b.id}', '${b.expiry_date}', '${itemName}')"
                                title="Edit Tanggal">
                            ✏️
                        </button>
                    </td>
...
// ======================== INPUT BARCODE ===================
Pastikan Laravel Echo sudah terinisialisasi di aplikasi Anda
Echo.channel('scanner-channel')
    .listen('.batch.scanned', (data) => {
        console.log("Barcode diterima dari Raspi:", data.barcode);

        // 1. CARI KOLOM FORM INPUT BARCODE DI WEB ANDA
        // Ganti 'barcode_input_id' dengan atribut id="" asli dari tag <input> form Anda
        let formInput = document.getElementById('barcode_input_id');

        if (formInput) {
            // Masukkan angka barcode ke dalam form input secara otomatis
            formInput.value = data.barcode;

            // 2. JIKA INGIN FORM OTOMATIS TERSUBMIT/TERKIRIM SETELAH TERISI:
        ...
        if (typeof Echo !== 'undefined') {
            Echo.channel('scanner-channel')
                .listen('.batch.scanned', (data) => {
                    ...
                    // Ganti 'barcode_input_id' dengan ID asli input form pencarian/tambah data Anda
                    let formInput = document.getElementById('barcode_input_id');

                    if (formInput) {
                        formInput.value = data.barcode;

                        // Opsional: Otomatis submit form setelah terisi otomatis
                        // formInput.form.submit();
                    }
                });
        } else {
            console.error('Laravel Echo belum terinisialisasi. Pastikan broadcast terkonfigurasi.');
        }
    });
